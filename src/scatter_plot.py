...
def getCourseContent(course_a, course_b):
    ...
    # Dictionary to collect results per house
...
def plotCourseScatter(course_data, course_a, course_b):
    ...
    # Loop through houses
    for house, df in course_data.items():
        if not df.empty:  # only plot if data exists
            plt.scatter(
                df[course_a],
                df[course_b],
                label=house,
                color=colors.get(house, "black"),  # fallback to black if house missing
                alpha=0.7,
                edgecolors="black"
            )
